...
from mem_leak_detection.base import  make_uniform_result_group, AnomalousGroup, AnomalousPoint
            ...
            while next_itr_idx <= len(self.change_point_indexes) - 1:
                ...
                if len(window_series.index) > 1:
                   # print(window_series)
                    ...
                    if trend is not None and trend['exit_time'] <= self.critical_time and trend['stop'] < X_copy.index[-1]:
                        ...
                        self.fit_trends.append(trend['prediction_trend'] )
                        # Keep all the found trend information
                        self.found_trends.append(trend)

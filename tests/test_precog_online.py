...
import time
class TestPrecogOnline:
    def __init__(self):
        self.name = 'Precog_Online'
        self.threshold = 0.69
        self.negative = False
    ...
    def execute(self, file):
        ...
        for row in range(0, len(dataset_true_labels.index)):
            dataset_test['True_Labels'][dataset_true_labels['Start'].values[row]:dataset_true_labels['End'].values[row]] = 1
        ...
        predicted_labels = []
        for i in range(0, len(dataset_test.Value.values)):
        ...
        if all(elem == 0 for elem in actual_labels) and all(elem == 0 for elem in predicted_labels):
            ...
            #print(file, "score:", f1score, "updated_score:", f1_score(actual_labels, dataset_test['Updated_Labels'].values))
            score = f1_score(actual_labels, dataset_test['Updated_Labels'].values)
            #print(predicted_labels)
        elif p1.prediction_trend is None and self.negative is True and \
                all(elem == 0 for elem in predicted_labels) is not True and all(elem == 0 for elem in actual_labels):
            score = 1
        else:
